...
public class RegexHelper {
	...
	public boolean isCellPhone(String str) {
		...
		if (result) {
			log.debug(String.format("(regex) -> '%s'(은)는 핸드폰 형식이 맞습니다.", str));
		} else {
			log.debug(String.format("(regex) -> '%s'(은)는 핸드폰 형식이 아닙니다.", str));
		}
		return result;
	}
