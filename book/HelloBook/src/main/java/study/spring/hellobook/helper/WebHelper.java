package study.spring.hellobook.helper;
...
public class WebHelper {
	...
	/**
	 * 싱글톤 객체가 생성될 때 호출되는 메서드로 JSP 의 주요 내장객체를 멤버변수에 연결한다.
	 * @param request
	 * @param response
	 */
	public void init(HttpServletRequest request, HttpServletResponse response) {
		...
		String url = request.getRequestURL().toString();	// 현재 URL 을 획득한다.
		String queryString = request.getQueryString();		// URL 에서 "?"이후의 GET 파라미터 문자열을 모두 가져온다.
		
		if (queryString != null) {							// 가져온 값이 있다면 URL 과 결합하여 완전한 URL 을 구성한다.
			url = url + "?" + queryString;
		}
	 ...
	 * 
	 * @param url - 이동할 페이지의 URL, Null 일 경우 이전페이지로 이동
	 * @param msg - 화면에 표시할 메세지, Null 일 경우 표시 안함
	 * @return ModelAndView
	 */
	...
	/**
	 * 파라미터로 받은 내용을 가상의 View 로 생성후 리턴한다.
	...
	public ModelAndView virtualView(final String body) {
		/** 가상의 View 를 익명 클래스 방식으로 생성하여 리턴 */
		// -> import org.springframework.web.servlet.View;
		// -> 
	 ...
	 * HTTP 상태코드는 200으로 설정하고, 결과 메세지는 "OK"라는 값을 설정하여
	 ...
	 * 를 호출한다.
	 * 
	 * JSON 으로 표현하고자 하는 컬렉션이 없지만 처리 결과는 성공인 경우 사용한다.
	 ...
	 * HTTP 상태코드는 500으로 설정하고, 결과 메세지는 파라미터로 전달되는 값을 설정하여
	 * JSON 문자열을 리턴한다.
	 * 
	 * Service 구현체에서 예외 발생시 전달하는 에러메세지 등을 JSON으로 표현할 때 사용
	 * 
	 * @param rt
	 * 
	 * @return Map<String, Object>
	 ...
	 * HTTP 상태코드는 400으로 설정하고, 결과 메세지는 파라미터로 전달되는 값을 설정하여
	 * JSON 문자열을 리턴한다.
	 * 
	 ...
	 * 이 때, 파일 덮어쓰기를 방지하기 위해 파일의 이름을 변경하고 이를 중복 검사를 수행한다.
